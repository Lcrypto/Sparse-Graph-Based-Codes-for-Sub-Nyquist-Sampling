...
void FrontEnd::computeDelays()
{
    if (config->isNoisy() || config->applyWindow())
    {
        ...
        if (config->needToUseMaximumLikelihoodDetection())
        {
            ...
            {
                int tempDelay = ((int) floor(signalLength*((ffast_real) (double(rand())/RAND_MAX)))) % signalLength;
	    ...
	    unsigned long long int skip = 1;
            ...
            {
		r = ((int) floor(signalLength*((ffast_real) (double(rand())/RAND_MAX)))) % signalLength;
